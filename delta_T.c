...
int main(int argc, char ** argv){
  ...
  FILE *F, *LOG;
  int i,j,k, n_x, n_y, n_z, NUM_BINS, curr_Pop, arg_offset,num_th, mu_bins;
  double dvdx, ave, *p_box, *k_ave, max_v_deriv, **p_kmu, **mu_ave;
  unsigned long long ct, *in_bin_ct, nonlin_ct, temp_ct, **in_kmu_bin_ct, ct_mu, box_ct;
  float nf, max, maxi, maxj, maxk, maxdvdx, min, mini, minj, mink, mindvdx;
  float k_x, k_y, k_z, k_mag, k_sq, k_floor, k_ceil, k_max, k_first_bin_ceil, k_factor, mu;
  float delta_mu, mu_ceil, mu_floor, mu_max;
...
// deallocate what we aren't using anymore 
 ...
 k_factor = 1.5;
  ...
  //P: need something similar for the mu bins 
  while (k_ceil < k_max){
    NUM_BINS++;
    k_floor=k_ceil;
    k_ceil*=k_factor;
  ...
  mu_max = 1.1;
  //mu_floor = -1.0;
  //mu_ceil = -1.0 + delta_mu;
  mu_floor = 0.0;
  mu_ceil = 0.0 + delta_mu;
  while (mu_ceil < mu_max){
    mu_bins++;
    mu_floor += delta_mu;
    mu_ceil += delta_mu;
  }

  printf("number of k bins is %d and number of mu bins is %d, mu_floor is %e and mu_ceil is %e \n", NUM_BINS, mu_bins, mu_floor, mu_ceil);
  ...
  //P: need to define something like this but for 2D arrays.
  //p_kmu = (double *)malloc(sizeof(double) *NUM_BINS *mu_bins);
  //***********************************************************HERE******************************************************************
  p_kmu = malloc(NUM_BINS*sizeof(double*));
  in_kmu_bin_ct = malloc(NUM_BINS*sizeof(unsigned long long*));
  mu_ave = malloc(NUM_BINS*sizeof(double*));
  int ii = 0;
  for (ii = 0; ii < NUM_BINS; ii++){
    p_kmu[ii] = malloc(mu_bins*sizeof(double));
    in_kmu_bin_ct[ii] = malloc(mu_bins*sizeof(unsigned long long));
    mu_ave[ii] = malloc(mu_bins*sizeof(double));
  }
  //mu_ave = (double *)malloc(sizeof(double)*mu_bins);
  //in_bin_mu_ct = (unsigned long long *)malloc(sizeof(unsigned long long)*mu_bins);
  //in_kmu_bin_ct = (unsigned long long *)malloc(sizeof(unsigned long long) *NUM_BINS *mu_bins);
  ...
  //******************************************************************HERE****************************************************************
  //P: I need to do something similar
  for (ct = 0; ct<NUM_BINS; ct++){
    k_ave[ct] = 0;
    in_bin_ct[ct] = 0;
    for (ct_mu = 0; ct_mu<mu_bins; ct_mu++){
      p_kmu[ct][ct_mu] = 0;
      in_kmu_bin_ct[ct][ct_mu] = 0;
      mu_ave[ct][ct_mu] = 0;
    }
  }
 // for (ct_mu = 0; ct_mu<mu_bins; ct_mu++){
   // mu_ave[ct_mu] = 0;
    //in_bin_mu_ct[ct_mu] = 0;
  //}
  ...
  if (!deldel_T){
    ...
    free(delta_T); fclose(LOG); free(p_box); free(k_ave); free(in_bin_ct); free(mu_ave); free(in_kmu_bin_ct); free(p_kmu);
    fftwf_cleanup_threads(); return -1;
  }
...
//P: So I want to probably change what is under here, in reality I actually want the deldel_T(k, mu) with mu = k_par/k_mag, note that k_par = k_z and k_mag = k_x^2 + k_y^2 + k_z^2
  ...
  for (n_x=0; n_x<HII_DIM; n_x++){
	...
	mu = k_z/(1.0*k_mag);
	// mu it's never negative... what should I do...
	//printf("the k_mag is %e while mu is %e \n", k_mag, mu);
	...
	while (k_ceil < k_max){
	  ...
	  if ((k_mag>=k_floor) && (k_mag < k_ceil)){
	    ...
	    while (mu_ceil < mu_max){
	      if ((mu>=mu_floor) && (mu < mu_ceil)){
		in_kmu_bin_ct[ct][ct_mu]++;
		p_kmu[ct][ct_mu] += pow(k_mag,3)*pow(cabs(deldel_T[HII_C_INDEX(n_x, n_y, n_z)]), 2)/(2.0*PI*PI*VOLUME);
		mu_ave[ct][ct_mu] += mu;
		//printf("uhmm... %e and mu %e, mu_floor is %e and mu_ceil is %e, with mu_final is %e \n", p_kmu[ct][ct_mu], mu_ave[ct_mu], mu_floor, mu_ceil, mu_max);
		break;
	      }
	      ct_mu++;
	      mu_floor = mu_ceil;
  ...
  if (!F){
    ...
    free(delta_T); fclose(LOG); free(p_box); free(k_ave); free(in_bin_ct); free(mu_ave); fftwf_free(deldel_T); free(p_kmu); free(in_kmu_bin_ct);
  }
  for (ct=1; ct<NUM_BINS; ct++){
    if (in_bin_ct[ct]>0){
      for (ct_mu = 1; ct_mu<mu_bins; ct_mu++){
	if (in_kmu_bin_ct[ct][ct_mu]>0){
	        fprintf(F, "%e\t%e\t%e\t%e\t%e\n", k_ave[ct]/(in_bin_ct[ct]+0.0), mu_ave[ct][ct_mu]/(in_kmu_bin_ct[ct][ct_mu]+0.0), p_box[ct]/(in_bin_ct[ct]+0.0), p_kmu[ct][ct_mu]/(in_kmu_bin_ct[ct][ct_mu]+0.0), p_box[ct]/(in_bin_ct[ct]+0.0)/sqrt(in_bin_ct[ct]+0.0));	  
//	        fprintf(F, "%e\t%e\t%e\t%e\t%e\n", k_ave[ct]/(in_bin_ct[ct]+0.0), mu_ave[ct], p_box[ct]/(in_bin_ct[ct]+0.0), p_kmu[ct][ct_mu]/(in_kmu_bin_ct[ct][ct_mu]+0.0), p_box[ct]/(in_bin_ct[ct]+0.0)/sqrt(in_bin_ct[ct]+0.0));
	}
      }
    }
  }
  fclose(F); free(p_box); free(k_ave); free(in_bin_ct); free(mu_ave); fftwf_free(deldel_T); free(p_kmu); free(in_kmu_bin_ct);
